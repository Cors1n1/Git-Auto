...
- **Gestão Gitignore:** Gerador inteligente com suporte a múltiplos templates e integração automática (incluindo exclusão de atalhos `.lnk`).
...
- Inclusão da regra de exclusão automática para arquivos `.lnk` no gerador de `.gitignore`.
...
- Implementado sistema de temas (Dracula, Nord, Matrix, etc) com persistência via .env e suporte a customização de cores via arquivo de configuração.
